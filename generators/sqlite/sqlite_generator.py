    ...
    def generate(self, language, translation):
        ...
        if 'footnotes' in prepared_data and len(prepared_data['footnotes']) > 0:
            ...
            for fn in prepared_data['footnotes']:
                cursor.execute(f"""
                INSERT INTO {translation}_footnotes (book, chapter, verse, catch_word, note_text, note_type)
                VALUES (?, ?, ?, ?, ?, ?);
                """, (fn['book'], fn['chapter'], fn['verse'], fn['catch_word'], fn['note_text'], fn['note_type']))
