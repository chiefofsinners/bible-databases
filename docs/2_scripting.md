...
#### `extract_sword_footnotes.py`
- **Description**: Extracts footnotes (marginal notes) from a SWORD module and adds them to the source JSON. Currently processes the KJV, which contains 6,959 footnotes with Hebrew/Greek literal translations and alternative English readings. The footnotes are stored both inline on each verse and as a top-level array for easy database import.
- **Usage**: Run the script to extract KJV footnotes from the SWORD module. The source JSON at `sources/en/KJV/KJV.json` will be updated in-place.
